...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def switch_features_handler(self, ev):
        ...
        if FLOWDEBUG:
            # stats_request = parser.OFPFlowStatsRequest(datapath=datapath)
            # datapath.send_msg(stats_request)
            datapath.send_msg(parser.OFPFlowStatsRequest(datapath=datapath))
        ...
        if NOJUNIPER:
            self.add_flow(datapath, 1, parser.OFPMatch(eth_type=0x0800,eth_src='50:c5:8d:ab:03:81'), [])
            self.add_flow(datapath, 1, parser.OFPMatch(eth_type=0x0806,eth_src='50:c5:8d:ab:03:81'), [])
        self.send_barrier_request(datapath)
        if FLOWDEBUG:
            # datapath.send_msg(stats_request)
            datapath.send_msg(parser.OFPFlowStatsRequest(datapath=datapath))

        # this is teh correct syntax for a mask, however HP doesn't support it in this context
        # self.add_flow(datapath, 1, parser.OFPMatch(eth_type=0x0800,eth_src=('50:c5:8d:00:00:00','ff:ff:ff:00:00:00')), [])
    ...
    def _packet_in_handler(self, ev):
        ...
        if out_port != ofproto.OFPP_FLOOD:
            match = parser.OFPMatch(eth_type=0x0800,in_port=in_port, eth_dst=dst)
